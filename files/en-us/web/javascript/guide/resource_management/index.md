...
> **Note:** While memory management and resource management are two separate topics, sometimes you can hook into the memory management system to do resource management, as a last resort. For example, you can create a {{jsxref("FinalizationRegistry")}} to clean up resources when the object is garbage collected. However, there is no guarantee that the finalizer will run, so it's not a good idea to rely on it for critical resources.
...
- Error handling: what happens if the `reader.read()` call fails? Then `readUntilB` would terminate and never get to the `reader.releaseLock()` call. We can fix this using {{jsxref("try...catch", "try...finally")}}:
...
You see how a seemingly benign task of calling `releaseLock` can quickly lead to nested boilerplate code. This is why we need integrated language support for resource management.
...
Now we know _when_ `using` does cleanup. But _how_ is it done? `using` requires the resource to implement the _disposable_ protocol, respectively. An object is disposable if it has the [`@@dispose`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol/dispose) method. This method is called with no arguments to perform cleanup. For example, in the reader case, the `@@dispose` property can be a simple alias or wrapper of `releaseLock`:
...
Every scope has a list of resources associated with it, in the order they were declared. When the scope exits, the resources are disposed in reverse order, by calling their `@@dispose` method.

`await using` is very similar to `using`. The name tells you that an `await` happens somewhere—not exactly when the resource is declared, but where it's disposed. To be clear, `await using` requires the resource to be _async disposable_, which means it has an [`@@asyncDisposable`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol/asyncDispose) method. This method is called with no arguments and returns a promise that resolves when the cleanup is done. Sometimes the cleanup is asynchronous, such as `filehandle.close()`. In this case, the result of the disposal can only be known asynchronously, so we need `@@asyncDispose`.
...
- `using` and `await using` are _opt in_. If you declare your resource using `let`, `const`, or `var`, no automatic disposal happens, just as before.
- `using` and `await using` require the resource to be disposable (or async disposable). If the resource does not have the `@@dispose` or `@@asyncDispose` method, you will get a `TypeError` at the line of declaration. The resource can be `null` or `undefined`, however, allowing you to conditionally acquire resources.
- Like `const`, `using` and `await using` variables cannot be re-assigned, but its properties can be changed. However, the `@@dispose`/`@@asyncDispose` method is already saved at the time of declaration, so changing the method after the declaration does not affect the cleanup.
...
Another use case is when you have a resource that does not yet implement the disposable protocol.

Yet another use case is when you have a disposal action to perform but it's not "tethered" to any resource in particular.
...
- The _disposable_ and _async disposable_ protocols, respectively, for resources to implement.
