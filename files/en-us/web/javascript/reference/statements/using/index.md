...
The **`using`** declaration declares block-scoped local variables that are _synchronously disposed_. Like {{jsxref("Statements/const", "const")}}, variables declared with `using` must be initialized and cannot be reassigned. The variable's value must be either `null`, `undefined`, or an object with an {{jsxref("Symbol/dispose", "@@dispose")}} method. When the variable goes out of scope, the `@@dispose` method of the object is called, to ensure that resources are freed.
...
- `valueN`
  - : Initial value of the variable. It can be any legal expression but its value must be either `null`, `undefined`, or an object with an {{jsxref("Symbol/dispose", "@@dispose")}} method.
...
- In the initializer of a [`for`](/en-US/docs/Web/JavaScript/Reference/Statements/for), [`for...of`](/en-US/docs/Web/JavaScript/Reference/Statements/for...of), or [`for await...of`](/en-US/docs/Web/JavaScript/Reference/Statements/for_await...of) loop
...
When the variable is first declared, a _disposer_ is retrieved from the object. If the `@@dispose` property doesn't contain a function, a `TypeError` is thrown. This disposer is saved to the scope.
...
In the following examples, we assume a simple `Resource` class that has a `getValue` method and a `@@dispose` method:
