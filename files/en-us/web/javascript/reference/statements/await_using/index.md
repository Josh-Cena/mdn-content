...
The **`await using`** declaration declares block-scoped local variables that are _asynchronously disposed_. Like {{jsxref("Statements/const", "const")}}, variables declared with `await using` must be initialized and cannot be reassigned. The variable's value must be either `null`, `undefined`, or an object with an {{jsxref("Symbol/asyncDispose", "@@asyncDispose")}} or {{jsxref("Symbol/dispose", "@@dispose")}} method. When the variable goes out of scope, the `@@asyncDispose` or `@@dispose` method of the object is called and awaited, to ensure that resources are freed.
...
- `valueN`
  - : Initial value of the variable. It can be any legal expression but its value must be either `null`, `undefined`, or an object with an {{jsxref("Symbol/asyncDispose", "@@asyncDispose")}} or {{jsxref("Symbol/dispose", "@@dispose")}} method.
...
- In the initializer of a [`for`](/en-US/docs/Web/JavaScript/Reference/Statements/for), [`for...of`](/en-US/docs/Web/JavaScript/Reference/Statements/for...of) (if the `for` loop is also in an async context), or [`for await...of`](/en-US/docs/Web/JavaScript/Reference/Statements/for_await...of) loop
...
When the variable is first declared, a _disposer_ is retrieved from the object. The `@@asyncDispose` property is tried first, and falls back to `@@dispose` if `@@asyncDispose` is `undefined`. If neither property contains a function, a `TypeError` is thrown. Notably, the `@@dispose` method is wrapped into a function that looks like `async () => { object[Symbol.dispose](); }`, which means if it returns a promise, that promise is _not_ awaited. This disposer is saved to the scope.

When the variable goes out of scope, the disposer is called and awaited. If the scope contains multiple `using` or `await using` declarations, all disposers are run in sequence in the reverse order of declaration, regardless of the type of declaration. All disposers are guaranteed to run (much like the `finally` block in `try...catch...finally`)—each time a disposer throws an exception (except the first time), a new {{jsxref("SuppressedError")}} is created, with the previous exception (which can either be the error causing scope-exit, or thrown from a previous disposer) as the `suppressed` property and the new exception as the `error` property.

The variable is allowed to have value `null` or `undefined`, so the resource can be optionally present. As long as one `await using` variable is declared in this scope, at least one `await` is guaranteed to happen on scope exit, even if the variable actually has value `null` or `undefined`. This prevents the disposal to end synchronously, causing timing issues.
